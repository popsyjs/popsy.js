...
Popsy.palette = function (commands, options = {}) {
  ...
  const getIcon = (icon) => { if (!icon) return defaultIcon; if (icon.trim().startsWith('<')) return icon; return icons[icon] || defaultIcon; };

  const overlay = document.createElement('div');
  overlay.style.cssText = `position:fixed;inset:0;background:rgba(0,0,0,0.45);backdrop-filter:blur(4px);z-index:99999;display:flex;align-items:flex-start;justify-content:center;padding-top:14vh;opacity:0;transition:opacity 0.15s ease;pointer-events:none;`;

  const modal = document.createElement('div');
  modal.style.cssText = `width:100%;max-width:560px;background:#1c1c1e;border:1px solid #2e2e32;border-radius:14px;overflow:hidden;box-shadow:0 24px 60px rgba(0,0,0,0.5);transform:scale(0.96) translateY(-6px);transition:transform 0.15s ease;font-family:sans-serif;`;
  ...
  input.type = 'text'; input.placeholder = placeholder; input.autocomplete = 'off'; input.spellcheck = false;
  ...
  searchRow.appendChild(input); searchRow.appendChild(escBadge);
  ...
  const fStyle = `display:flex;align-items:center;gap:4px;font-size:11px;color:#444;font-family:monospace;`;
  const kStyle = `background:#2a2a2e;border:1px solid #3a3a3e;border-radius:3px;padding:1px 5px;color:#555;font-size:10px;`;
  footer.innerHTML = `<span style="${fStyle}"><kbd style="${kStyle}">↑</kbd><kbd style="${kStyle}">↓</kbd> navigate</span><span style="${fStyle}"><kbd style="${kStyle}">↵</kbd> run</span><span style="${fStyle}"><kbd style="${kStyle}">esc</kbd> close</span>`;

  modal.appendChild(searchRow); modal.appendChild(list); modal.appendChild(footer);
  ...
  const render = () => {
    ...
    if (filtered.length === 0) {
      ...
      e.style.cssText = `padding:40px 20px;text-align:center;color:#555;font-size:14px;`;
      e.textContent = 'No results found'; list.appendChild(e); return;
    }
    const groups = {};
    filtered.forEach(cmd => { const g = cmd.category || 'General'; if (!groups[g]) groups[g] = []; groups[g].push(cmd); });
    let idx = 0;
    for (const [group, items] of Object.entries(groups)) {
      const gl = document.createElement('div');
      gl.style.cssText = `font-size:10px;color:#555;letter-spacing:0.1em;text-transform:uppercase;padding:10px 12px 4px;font-family:monospace;`;
      gl.textContent = group; list.appendChild(gl);
      items.forEach(cmd => {
        const i = idx++; const active = i === activeIdx;
        ...
        iw.style.cssText = `width:32px;height:32px;background:${active ? 'rgba(255,255,255,0.15)' : '#2a2a2e'};border-radius:8px;display:flex;align-items:center;justify-content:center;flex-shrink:0;color:${active ? '#fff' : '#aaa'};`;
        iw.innerHTML = getIcon(cmd.icon);
        const lbl = document.createElement('div');
        lbl.style.cssText = `flex:1;font-size:14px;color:${active ? '#fff' : '#e0e0e0'};`;
        lbl.textContent = cmd.label;
        const rgt = document.createElement('div');
        rgt.style.cssText = `font-size:11px;font-family:monospace;color:${active ? '#c7d2fe' : '#555'};background:${active ? 'rgba(255,255,255,0.1)' : '#2a2a2e'};border:1px solid ${active ? '#818cf8' : '#3a3a3e'};border-radius:4px;padding:2px 7px;`;
        rgt.textContent = cmd.shortcut || (cmd.category || 'General');
        item.appendChild(iw); item.appendChild(lbl); item.appendChild(rgt);
        item.addEventListener('mouseenter', () => { activeIdx = i; render(); });
        item.addEventListener('click', () => { cmd.action(); close(); });
        list.appendChild(item);
        if (active) setTimeout(() => item.scrollIntoView({ block: 'nearest' }), 0);
      });
    }
  };
  ...
  const filter = (q) => {
    ...
    filtered = query ? commands.filter(c => c.label.toLowerCase().includes(query) || (c.category || '').toLowerCase().includes(query)) : [...commands];
    activeIdx = 0; render();
  };
